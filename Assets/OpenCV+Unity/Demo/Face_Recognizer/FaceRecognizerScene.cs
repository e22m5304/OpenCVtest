namespace OpenCvSharp.Demo
{
	...
	{
		public UnityEngine.Texture2D rawColorImg;
		...
		{
			// サンプル画像を含むディレクトリのパス
			string sampleImagesDirectory = @"C:\Users\amuro\OpenCVtest\Assets\testFace";

			// TrainRecognizerメソッドを呼び出して認識器をトレーニング
			TrainRecognizer(sampleImagesDirectory);

			// convert texture to cv image
			Mat image = Unity.TextureToMat(this.rawColorImg);
