...
public class FaceParticle : MonoBehaviour
{
        ...
        while (true)
        {
            using (Capture capture = await Task.Run(() => this.kinect.GetCapture()).ConfigureAwait(true))
            {
                ...
                if (frame.NumberOfBodies > 0)
                {
                    ...
                    ////加速度判定
                    if (0.3 < GetAccerareta().magnitude && currentframe > 60)
                    {
                    ...
                    ////加速度判定
                    if (0.12 < GetrAccerareta().magnitude && currentframe > 60 &&GetrVelocity().z < -5)
                    {
                    ...
                    Debug.Log("S" + GetrVelocity().z);

                    Debug.Log("A" + GetrAccerareta().magnitude);
                    ////true
                    if (risThrow)
                    {
