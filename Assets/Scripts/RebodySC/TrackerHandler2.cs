...
public class TrackerHandler2 : MonoBehaviour
{
    ...
    [SerializeField] private float[] savedBodyData = new float[(int)JointId.Count];
    [SerializeField] private bool[] sameParts = new bool[(int)JointId.Count];
    [SerializeField] private bool save = false;
    [SerializeField] private Material red;
    [SerializeField] private Material blue;
    [SerializeField] private Material joint;
    [SerializeField] [Range(0f, 1f)] private float threshold = 0.03f;
    [SerializeField] private Slider _slider;

    // Start is called before the first frame update
    void Awake()
    {
        _slider.value = threshold;
    ...
    public void updateTracker(BackgroundData trackerFrameData)
    {
        ...
        {
            var length = (skeleton.JointPositions3D[i] - skeleton.JointPositions3D[(int)parentJointMap[(JointId)i]]).Length();

            if (Mathf.Abs(savedBodyData[i] - length) < threshold)
            {
                sameParts[i] = true;
            }
            else
    ...
    public void renderSkeleton(Body skeleton, int skeletonNumber)
    {
        for (int jointNum = 0; jointNum < (int)JointId.Count; jointNum++)
        {
            ...
            transform.GetChild(skeletonNumber).GetChild(jointNum).GetComponent<MeshRenderer>().material = joint;
            ...
            if (parentJointMap[(JointId)jointNum] != JointId.Count)
            {
                ...
                if (sameParts[jointNum])
                {
                    transform.GetChild(skeletonNumber).GetChild(jointNum).GetChild(boneChildNum)
